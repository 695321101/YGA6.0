...
YGA（Your Generated App）是一套 AI 软件生成流水线。它希望让 **不懂代码的人** 也能表达需求、确认结果并得到可运行的软件；同时也不排斥程序员，程序员可以参与底层能力建设、质量审核、复杂问题处理和后续扩展。

> 代码编辑器解决的是「会写代码的人如何更高效地写代码」；YGA 更关注「一个软件想法如何被记录、规划、生成、检查、评审，并最终交付」。
