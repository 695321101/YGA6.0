...
YGA 不是一个简单的「把需求发给 AI，然后等它吐代码」的工具。它的核心是一条 **可记录、可检查、可返工** 的软件生成流水线。
...
| **YGA Studio 浏览器（前端线）** | 未来让用户在生成页面上点选元素，用自然语言改界面 | 浏览器不是独立玩具，而是后续前端线的人机协作入口 |

这条链路的目标是：让普通人只负责表达需求和确认结果；需求记忆、规划、契约、生成、检查、评审和可视化修改由 YGA 承担。

当前开源仓库先放出 **Python 后端流水线核心**。MCP 浏览器 / YGA Studio 浏览器属于同一产品线的后续前端能力：它会把这条流水线从命令行和脚本，带到一个普通人能直接操作的界面里。
...
后续更新将开启 **前端线**。它不是另一个无关项目，而是把当前后端流水线交给普通人使用的界面层，包含：
